...
function save_parcel(){
	...
	foreach($price as $k => $v){
		...
		if(empty($id)){
			$i = 0;
			while($i == 0){
				$ref = 'EEE' . sprintf("%'09d", mt_rand(0, 999999999));
	...
	function update_parcel(){
      ...
      function update_history() {
		...
		$update = $this->db->query("INSERT INTO `shipping_history` (parcel_id, status, location, comments, timestamp) VALUES ('$id', '$status', '$location', '$comments', '$timestamp')");
		$updateParcel = $this->db->query("UPDATE parcels SET status='$status' WHERE id='$id'");
		$save = $this->db->query("UPDATE parcel_tracks SET status='$status' WHERE parcel_id='$id'");
	
		if ($update && $save && $updateParcel) {
			// Fetch receiver email and other details
			$parcelResult = $this->db->query("SELECT recipient_email, recipient_name, reference_number FROM parcels WHERE id='$id'");
			if ($parcelResult) {
				...
				error_log("Failed to fetch parcel details: " . $this->db->error);
				return 0;
			}
	
			// Fetch the latest status from shipping history
			$historyResult = $this->db->query("SELECT status FROM shipping_history WHERE parcel_id='$id' ORDER BY id DESC LIMIT 1");
			if ($historyResult) {
				$history = $historyResult->fetch_assoc();
				$latest_status = $history['status'];
			} else {
				error_log("Failed to fetch shipping history: " . $this->db->error);
				return 0;
			}
...
<body>
    <div class='container'>
        <div class='header'>
           <img src='../easyline-logo.png' alt='Easylinexpressdelivery' />
        </div>
        <div class='content'>
            <h4>Hello {$recipient_name},</h4>
            <p>We are pleased to inform you that your shipment is in progress. We will notify you when your parcel arrives.</p>
            <h2 class='tracking-info'>Tracking Information</h2>
            <p>Tracking Number: {$reference_number}</p>
            <p>Location: {$location}</p>
            <p>Comments: {$comments}</p>
            <p>We hope this email meets your approval. Please do not hesitate to get in touch via email info@easylinexpressdelivery.com or visit <a href='https://easylinexpressdelivery.com/'>Easylinexpressdelivery.</a> if we can be of any further assistance.</p>
            <p>Yours sincerely, <br/> Support Team at <a href='https://easylinexpressdelivery.com/'>Easyline Express Delivery.</a></p>
            <p></p>
        </div>
        <div class='footer'>
            <p>&copy; 2024 Easyline Express Delivery.</a></p>
        </div>
    </div>
</body>
</html>";
			...
			$headers .= "From: Easylinexpressdelivery <noreply@easylinexpressdelivery.com>\r\n";
	
			// Send the email
			if (mail($recipient_email, $subject, $message, $headers)) {
				return 1;
			} else {
				error_log("Email failed to send to $recipient_email");
				return 0;
			}
		} else {
			if (!$update) {
				error_log("Failed to insert into shipping_history: " . $this->db->error);
			}
			if (!$save) {
				error_log("Failed to update parcel_tracks: " . $this->db->error);
			}
			if (!$updateParcel) {
				error_log("Failed to update parcels: " . $this->db->error);
			}
		}
		return 0;
	}
